...
> 当前阶段：**W2 后端开发完成（代码层验证全绿），W3 待启动**；⚠️ W2 **飞书集成层待测**（MR-01 监控推群 / FS-* 需你本地实测），W3 详细计划已 re-plan 展开（见 `开发记录与计划.md` W3 章）待你确认；Aily 仅作参考，主通道为代码 Webhook（见 `08` + `开发记录与计划.md`）。
...
| **W2 开发（v3，监控优先）** | 监控雷达（唯一硬差异化）+ 结构化分析引擎 + 优雅降级 + 中文源占位；v3 修订（G6降级/监控提前/中文源占位） | **代码层全 PASS**（端到端 DDG 3 条 + 1778 字带引用；pricing 红线 G1 合规）；`06` 表追加 5 条 W2 PASS 行 | `af8a4a4` + `017b164` + `ffca813` + `cc7d9d8` | **飞书集成待测**：MR-01 监控推群 / FS-* 需你本地起服务+ngrok+飞书群实测 |
| **W3 待启动（Gate 待确认）** | 飞书卡片渲染 + `/compare` 对比 + 会话 SQLite 重建 + webhook 加固 | 详细计划已 re-plan 展开（W3 章）；3.1 card_renderer 代码写好**待提交**；门禁=先等 W2 飞书实测闭环再提交 W3.1 | — | 见 `开发记录与计划.md` W3 章（含前置门禁） |
...
- [x] **W2 验证已落库（2026-07-25）**：详见 `开发记录与计划.md` W2 章 §4——代码层/引擎冒烟/红线回归全绿（真实 DDG 搜索 3 条 + DeepSeek 1778 字带引用；pricing 红线 G1 合规）；`06-测试记录表.csv` 追加 5 条 W2 代码层 PASS 行（W2-IMP/E2E/RED/RBAC/CLN）。**飞书集成项（MR-01/FS-*）仍待测**，需用户本地起服务 + ngrok + 飞书群实测。
- [ ] **W3 待启动（经 Re-plan Gate）**：详细计划已展开于 `开发记录与计划.md` W3 章（含 W2 飞书集成门禁）；3.1 `card_renderer.py` 代码已写好待提交（不跳过 Gate）；**门禁决策（2026-07-25 11:20）：先等 W2 飞书集成本地实测闭环（MR-01）后再提交 W3.1**。按 v3 修正（来源分色降级为仅 🔵通用、监控已前置不重复）。
