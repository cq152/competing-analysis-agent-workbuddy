...
| **W3 待启动（Gate 待确认）** | 飞书卡片渲染 + `/compare` 对比 + 会话 SQLite 重建 + webhook 加固 | 详细计划已 re-plan 展开（W3 章）；3.1 card_renderer 代码写好**待提交**；须先确认 W2 飞书门禁策略 | — | 见 `开发记录与计划.md` W3 章（含前置门禁） |
...
- [ ] **W3 待启动（经 Re-plan Gate）**：详细计划已展开于 `开发记录与计划.md` W3 章（含 W2 飞书集成门禁）；3.1 `card_renderer.py` 代码已写好待提交（不跳过 Gate）；下一步待你确认计划 + W2 飞书实测策略。按 v3 修正（来源分色降级为仅 🔵通用、监控已前置不重复）。
