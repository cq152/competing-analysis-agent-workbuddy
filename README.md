...
> 当前阶段：**W2 后端开发完成（代码层验证全绿），W3 待启动**；⚠️ W2 **飞书集成层待测**（MR-01 监控推群 / FS-* 需你本地实测），W3 详细计划已 re-plan 展开（见 `W2-开发记录与验收.md` §5.1）待你确认；Aily 仅作参考，主通道为代码 Webhook（见 `08` + `W2-开发记录与验收.md`）。
...
| `W2-开发记录与验收.md` | **W2 阶段单一记录**：计划(v3)+执行手册+审计+验证+W3/W4计划（合并原 09/11/15/16/17） | W2 开发 |
...
> **总验收报告**：本文件是项目单一事实来源兼总验收报告。每个阶段结束在此写一段验收结论（做了什么 + 验证结果 + commit + 遗留）。详细阶段记录见各阶段文档（W1→`12`；W2→`W2-开发记录与验收.md`）。
...
| **W3 待启动（Gate 待确认）** | 飞书卡片渲染 + `/compare` 对比 + 会话 SQLite 重建 + webhook 加固 | 详细计划已 re-plan 展开（§5.1）；3.1 card_renderer 代码写好**待提交**；须先确认 W2 飞书门禁策略 | — | 见 `W2-开发记录与验收.md` §5.1（含前置门禁） |
...
- [x] **W2 v3 正式开发完成（2026-07-25）**：监控雷达（唯一硬差异化）+ 结构化分析引擎 + 优雅降级 + 中文源占位接口全部落地（commit `af8a4a4` + `017b164`），详见 `W2-开发记录与验收.md`。**下一步进入 W3**：按 `W2-开发记录与验收.md` §5 滚动计划细化（监控变化类型识别 / 周报自动进群 / 中文源实接依 W5 验证决定 / 飞书 Interactive Card）；或先跑真实用户验证（用户当前无 SMB 渠道，需先建渠道找 5–8 个试用）。
- [x] **计划 v3 修订（2026-07-25）**：基于 `13` 批判性审视 + `14` 竞品对比实验，落实用户决策——① 不做 concierge 预验证（先开发完再验证）② **G6 红线降级为目标** ③ **监控推送提前到 W2**（唯一硬差异化，详见 `14`）④ 中文数据源降为占位 ⑤ **定价重做**（免费层不送监控，送限次分析）。修订落地于 `W2-开发记录与验收.md` §一 + `04` §五 v2 + `06` G6 降级标注。**开发以 `W2-开发记录与验收.md` §一 v3 任务清单为准。**
...
- [ ] **W3 待启动（经 Re-plan Gate）**：详细计划已展开于 `W2-开发记录与验收.md` §5.1（含 W2 飞书集成门禁）；3.1 `card_renderer.py` 代码已写好待提交（不跳过 Gate）；下一步待你确认计划 + W2 飞书实测策略。按 v3 修正（来源分色降级为仅 🔵通用、监控已前置不重复）。
