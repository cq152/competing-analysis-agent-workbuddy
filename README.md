...
### 🆕 通用问答兜底（2026-07-26 加入）
- **触发**：用户在群里 @ 智能体，但消息与竞品分析无关、又属 AI 助手能帮忙的请求（数据分析、解释概念、写代码、总结、翻译、**联网查询最新信息**等）。
- **行为**：`classify_intent` 新增 `chat` 意图 → 走 `engine.general_qa()`（默认复用 `searcher.py` 的 DuckDuckGo 联网检索，失败优雅退化纯模型知识）→ 蓝色「💬 通用问答」卡片回复。
- **不误伤**：UI bug 类（按钮/没效果/查看详情/报错等）仍先被 `_OFF_TOPIC_RE` 正则拦下走澄清卡，不会硬凑分析卡也不会被当成通用问答。
